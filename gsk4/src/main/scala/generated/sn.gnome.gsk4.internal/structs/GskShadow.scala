...
*/
opaque type GskShadow = CStruct4[_root_.sn.gnome.gdk4.internal.GdkRGBA, Float, Float, Float]

object GskShadow:
  given _tag: Tag[GskShadow] = Tag.materializeCStruct4Tag[_root_.sn.gnome.gdk4.internal.GdkRGBA, Float, Float, Float]
  ...
  def apply(color : _root_.sn.gnome.gdk4.internal.GdkRGBA, dx : Float, dy : Float, radius : Float)(using Zone): Ptr[GskShadow] =
    val ____ptr = apply()
    (!____ptr).color = color
    (!____ptr).dx = dx
    (!____ptr).dy = dy
