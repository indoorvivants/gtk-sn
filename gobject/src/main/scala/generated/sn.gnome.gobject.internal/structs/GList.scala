...
opaque type GList = CStruct3[_root_.sn.gnome.glib.internal.gpointer, Ptr[Byte], Ptr[Byte]]

object GList:
  given _tag: Tag[GList] = Tag.materializeCStruct3Tag[_root_.sn.gnome.glib.internal.gpointer, Ptr[Byte], Ptr[Byte]]
  ...
  def apply(data : _root_.sn.gnome.glib.internal.gpointer, next : Ptr[_root_.sn.gnome.glib.internal.GList], prev : Ptr[_root_.sn.gnome.glib.internal.GList])(using Zone): Ptr[GList] =
    val ____ptr = apply()
    (!____ptr).data = data
    (!____ptr).next = next
    (!____ptr).prev = prev
