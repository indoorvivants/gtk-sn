...
*/
opaque type _GQueue = CStruct3[Ptr[Byte], Ptr[Byte], _root_.sn.gnome.glib.internal.guint]

object _GQueue:
  given _tag: Tag[_GQueue] = Tag.materializeCStruct3Tag[Ptr[Byte], Ptr[Byte], _root_.sn.gnome.glib.internal.guint]
  ...
  def apply(head : Ptr[_root_.sn.gnome.glib.internal.GList], tail : Ptr[_root_.sn.gnome.glib.internal.GList], length : _root_.sn.gnome.glib.internal.guint)(using Zone): Ptr[_GQueue] =
    val ____ptr = apply()
    (!____ptr).head = head
    (!____ptr).tail = tail
