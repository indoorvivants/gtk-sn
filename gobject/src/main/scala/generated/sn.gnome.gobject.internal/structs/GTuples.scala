...
opaque type GTuples = CStruct1[_root_.sn.gnome.glib.internal.guint]

object GTuples:
  given _tag: Tag[GTuples] = Tag.materializeCStruct1Tag[_root_.sn.gnome.glib.internal.guint]
  ...
  def apply(len : _root_.sn.gnome.glib.internal.guint)(using Zone): Ptr[GTuples] =
    val ____ptr = apply()
    (!____ptr).len = len
    ____ptr