...
trait Converter:
  ...
  /** This is the main operation used when converting data. It is to be called
    ...
    * producing some output (in @outbuf) or consuming some input (from @inbuf)
    * or both. If its not possible to do any work an error is returned.
    *
    ...
    * function again with the same data but with the %G_CONVERTER_INPUT_AT_END
    * flag set. This may cause the conversion to finish as e.g. in the regexp
    * match case (or, to fail again with %G_IO_ERROR_PARTIAL_INPUT in e.g. a
    * charset conversion where the input is actually partial).
    *
    ...
    * to write out all internal state to the output. The application has to call
    ...
    * returned instead of %G_CONVERTER_CONVERTED. This is somewhat similar to
    * what happens at the end of the input stream, but done in the middle of the
    * data.
    *
  ...
  /** Resets all internal state in the converter, making it behave as if it was
    ...
    */
  def reset(): Unit /* None */ =
    g_converter_reset(this.getUnsafeRawPointer().asInstanceOf[Ptr[GConverter]])
  end reset
