...
object _GDBusInterfaceSkeleton:
  ...
  // Allocates _GDBusInterfaceSkeleton on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GDBusInterfaceSkeleton] = scala.scalanative.unsafe.alloc[_GDBusInterfaceSkeleton](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GDBusInterfaceSkeletonPrivate])(using Zone): Ptr[_GDBusInterfaceSkeleton] =
    val ____ptr = apply()
