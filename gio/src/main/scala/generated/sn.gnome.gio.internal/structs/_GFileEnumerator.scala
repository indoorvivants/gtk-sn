...
object _GFileEnumerator:
  ...
  private[internal] object fields:
    extension (struct: _GFileEnumerator)
      ...
      inline def priv : Ptr[GFileEnumeratorPrivate] = struct._2
      inline def priv_=(value: Ptr[GFileEnumeratorPrivate]): Unit = (!struct.at2 = value)
    end extension
  
  // Allocates _GFileEnumerator on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GFileEnumerator] = scala.scalanative.unsafe.alloc[_GFileEnumerator](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GFileEnumeratorPrivate])(using Zone): Ptr[_GFileEnumerator] =
    val ____ptr = apply()
