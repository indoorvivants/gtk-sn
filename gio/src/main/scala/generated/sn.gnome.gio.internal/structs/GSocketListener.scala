...
object GSocketListener:
  ...
  private[internal] object fields:
    extension (struct: GSocketListener)
      ...
      inline def priv: Ptr[GSocketListenerPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSocketListenerPrivate]]]
      inline def priv_=(value: Ptr[GSocketListenerPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSocketListenerPrivate]]] = value
    end extension
  
  // Allocates GSocketListener on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GSocketListener] = scala.scalanative.unsafe.alloc[GSocketListener](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GSocketListenerPrivate])(using Zone): Ptr[GSocketListener] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GSocketListenerPrivate]].toInt)
    res
  end offsets