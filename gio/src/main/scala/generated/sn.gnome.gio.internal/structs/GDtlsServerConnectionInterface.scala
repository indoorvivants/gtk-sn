...
object GDtlsServerConnectionInterface:
  ...
  // Allocates GDtlsServerConnectionInterface on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GDtlsServerConnectionInterface] = scala.scalanative.unsafe.alloc[GDtlsServerConnectionInterface](1)
  def apply(g_iface : _root_.sn.gnome.gobject.internal.GTypeInterface)(using Zone): Ptr[GDtlsServerConnectionInterface] =
    val ____ptr = apply()
