...
object GSocket:
  ...
  private[internal] object fields:
    extension (struct: GSocket)
      ...
      inline def priv: Ptr[GSocketPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSocketPrivate]]]
      inline def priv_=(value: Ptr[GSocketPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSocketPrivate]]] = value
    end extension
  
  // Allocates GSocket on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GSocket] = scala.scalanative.unsafe.alloc[GSocket](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GSocketPrivate])(using Zone): Ptr[GSocket] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GSocketPrivate]].toInt)
    res
  end offsets