...
object GDtlsClientConnectionInterface:
  ...
  // Allocates GDtlsClientConnectionInterface on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GDtlsClientConnectionInterface] = scala.scalanative.unsafe.alloc[GDtlsClientConnectionInterface](1)
  def apply(g_iface : _root_.sn.gnome.gobject.internal.GTypeInterface)(using Zone): Ptr[GDtlsClientConnectionInterface] =
    val ____ptr = apply()
