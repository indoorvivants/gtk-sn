...
object GSettings:
  ...
  private[internal] object fields:
    extension (struct: GSettings)
      ...
      inline def priv: Ptr[GSettingsPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSettingsPrivate]]]
      inline def priv_=(value: Ptr[GSettingsPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSettingsPrivate]]] = value
    end extension
  
  // Allocates GSettings on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GSettings] = scala.scalanative.unsafe.alloc[GSettings](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GSettingsPrivate])(using Zone): Ptr[GSettings] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GSettingsPrivate]].toInt)
    res
  end offsets