...
object GCancellableClass:
  ...
  private[internal] object fields:
    extension (struct: GCancellableClass)
      ...
      inline def cancelled: CFuncPtr1[Ptr[GCancellable], Unit] = !struct.at(offsets(1)).asInstanceOf[Ptr[CFuncPtr1[Ptr[GCancellable], Unit]]]
      inline def cancelled_=(value: CFuncPtr1[Ptr[GCancellable], Unit]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[CFuncPtr1[Ptr[GCancellable], Unit]]] = value
  ...
  // Allocates GCancellableClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GCancellableClass] = scala.scalanative.unsafe.alloc[GCancellableClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass, cancelled : CFuncPtr1[Ptr[GCancellable], Unit], _g_reserved1 : CFuncPtr0[Unit], _g_reserved2 : CFuncPtr0[Unit], _g_reserved3 : CFuncPtr0[Unit], _g_reserved4 : CFuncPtr0[Unit], _g_reserved5 : CFuncPtr0[Unit])(using Zone): Ptr[GCancellableClass] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObjectClass].toInt, alignmentof[CFuncPtr1[Ptr[GCancellable], Unit]].toInt)
