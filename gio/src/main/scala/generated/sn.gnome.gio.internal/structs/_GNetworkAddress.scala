...
object _GNetworkAddress:
  ...
  private[internal] object fields:
    extension (struct: _GNetworkAddress)
      ...
      inline def priv : Ptr[GNetworkAddressPrivate] = struct._2
      inline def priv_=(value: Ptr[GNetworkAddressPrivate]): Unit = (!struct.at2 = value)
    end extension
  
  // Allocates _GNetworkAddress on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GNetworkAddress] = scala.scalanative.unsafe.alloc[_GNetworkAddress](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GNetworkAddressPrivate])(using Zone): Ptr[_GNetworkAddress] =
    val ____ptr = apply()
