...
object GTcpWrapperConnection:
  ...
  private[internal] object fields:
    extension (struct: GTcpWrapperConnection)
      inline def parent_instance: GTcpConnection = !struct.at(offsets(0)).asInstanceOf[Ptr[GTcpConnection]]
      inline def parent_instance_=(value: GTcpConnection): Unit = !struct.at(offsets(0)).asInstanceOf[Ptr[GTcpConnection]] = value
      inline def priv: Ptr[GTcpWrapperConnectionPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTcpWrapperConnectionPrivate]]]
      inline def priv_=(value: Ptr[GTcpWrapperConnectionPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTcpWrapperConnectionPrivate]]] = value
    end extension
  
  // Allocates GTcpWrapperConnection on the heap – fields are not initalised or zeroed out
