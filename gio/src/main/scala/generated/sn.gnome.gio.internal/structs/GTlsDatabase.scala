...
object GTlsDatabase:
  ...
  private[internal] object fields:
    extension (struct: GTlsDatabase)
      ...
      inline def priv: Ptr[GTlsDatabasePrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTlsDatabasePrivate]]]
      inline def priv_=(value: Ptr[GTlsDatabasePrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTlsDatabasePrivate]]] = value
    end extension
  
  // Allocates GTlsDatabase on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GTlsDatabase] = scala.scalanative.unsafe.alloc[GTlsDatabase](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GTlsDatabasePrivate])(using Zone): Ptr[GTlsDatabase] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GTlsDatabasePrivate]].toInt)
    res
  end offsets