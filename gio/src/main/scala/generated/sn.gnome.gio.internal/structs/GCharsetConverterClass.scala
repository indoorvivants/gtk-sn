...
object GCharsetConverterClass:
  ...
  // Allocates GCharsetConverterClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GCharsetConverterClass] = scala.scalanative.unsafe.alloc[GCharsetConverterClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass)(using Zone): Ptr[GCharsetConverterClass] =
    val ____ptr = apply()
