...
object _GSocketAddress:
  ...
  // Allocates _GSocketAddress on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GSocketAddress] = scala.scalanative.unsafe.alloc[_GSocketAddress](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject)(using Zone): Ptr[_GSocketAddress] =
    val ____ptr = apply()
