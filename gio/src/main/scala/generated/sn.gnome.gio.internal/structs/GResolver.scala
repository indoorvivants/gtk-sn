...
object GResolver:
  ...
  private[internal] object fields:
    extension (struct: GResolver)
      ...
      inline def priv: Ptr[GResolverPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GResolverPrivate]]]
      inline def priv_=(value: Ptr[GResolverPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GResolverPrivate]]] = value
    end extension
  
  // Allocates GResolver on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GResolver] = scala.scalanative.unsafe.alloc[GResolver](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GResolverPrivate])(using Zone): Ptr[GResolver] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GResolverPrivate]].toInt)
    res
  end offsets