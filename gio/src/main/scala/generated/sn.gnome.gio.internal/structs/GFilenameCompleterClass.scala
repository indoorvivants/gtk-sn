...
object GFilenameCompleterClass:
  ...
  private[internal] object fields:
    extension (struct: GFilenameCompleterClass)
      ...
      inline def got_completion_data: CFuncPtr1[Ptr[GFilenameCompleter], Unit] = !struct.at(offsets(1)).asInstanceOf[Ptr[CFuncPtr1[Ptr[GFilenameCompleter], Unit]]]
      inline def got_completion_data_=(value: CFuncPtr1[Ptr[GFilenameCompleter], Unit]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[CFuncPtr1[Ptr[GFilenameCompleter], Unit]]] = value
  ...
  // Allocates GFilenameCompleterClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GFilenameCompleterClass] = scala.scalanative.unsafe.alloc[GFilenameCompleterClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass, got_completion_data : CFuncPtr1[Ptr[GFilenameCompleter], Unit], _g_reserved1 : CFuncPtr0[Unit], _g_reserved2 : CFuncPtr0[Unit], _g_reserved3 : CFuncPtr0[Unit])(using Zone): Ptr[GFilenameCompleterClass] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObjectClass].toInt, alignmentof[CFuncPtr1[Ptr[GFilenameCompleter], Unit]].toInt)
