...
object GOutputStream:
  ...
  private[internal] object fields:
    extension (struct: GOutputStream)
      ...
      inline def priv: Ptr[GOutputStreamPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GOutputStreamPrivate]]]
      inline def priv_=(value: Ptr[GOutputStreamPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GOutputStreamPrivate]]] = value
    end extension
  
  // Allocates GOutputStream on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GOutputStream] = scala.scalanative.unsafe.alloc[GOutputStream](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GOutputStreamPrivate])(using Zone): Ptr[GOutputStream] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GOutputStreamPrivate]].toInt)
    res
  end offsets