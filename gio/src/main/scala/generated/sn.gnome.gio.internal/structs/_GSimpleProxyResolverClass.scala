...
object _GSimpleProxyResolverClass:
  ...
  // Allocates _GSimpleProxyResolverClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GSimpleProxyResolverClass] = scala.scalanative.unsafe.alloc[_GSimpleProxyResolverClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass, _g_reserved1 : CFuncPtr0[Unit], _g_reserved2 : CFuncPtr0[Unit], _g_reserved3 : CFuncPtr0[Unit], _g_reserved4 : CFuncPtr0[Unit], _g_reserved5 : CFuncPtr0[Unit])(using Zone): Ptr[_GSimpleProxyResolverClass] =
    val ____ptr = apply()
