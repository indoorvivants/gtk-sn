...
object GInetAddressMask:
  ...
  private[internal] object fields:
    extension (struct: GInetAddressMask)
      ...
      inline def priv: Ptr[GInetAddressMaskPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GInetAddressMaskPrivate]]]
      inline def priv_=(value: Ptr[GInetAddressMaskPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GInetAddressMaskPrivate]]] = value
    end extension
  
  // Allocates GInetAddressMask on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GInetAddressMask] = scala.scalanative.unsafe.alloc[GInetAddressMask](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GInetAddressMaskPrivate])(using Zone): Ptr[GInetAddressMask] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GInetAddressMaskPrivate]].toInt)
    res
  end offsets