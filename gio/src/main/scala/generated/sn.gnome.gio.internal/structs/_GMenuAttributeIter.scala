...
object _GMenuAttributeIter:
  ...
  private[internal] object fields:
    extension (struct: _GMenuAttributeIter)
      ...
      inline def priv : Ptr[GMenuAttributeIterPrivate] = struct._2
      inline def priv_=(value: Ptr[GMenuAttributeIterPrivate]): Unit = (!struct.at2 = value)
    end extension
  
  // Allocates _GMenuAttributeIter on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GMenuAttributeIter] = scala.scalanative.unsafe.alloc[_GMenuAttributeIter](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GMenuAttributeIterPrivate])(using Zone): Ptr[_GMenuAttributeIter] =
    val ____ptr = apply()
