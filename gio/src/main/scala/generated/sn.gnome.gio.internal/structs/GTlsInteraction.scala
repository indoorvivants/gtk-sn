...
object GTlsInteraction:
  ...
  private[internal] object fields:
    extension (struct: GTlsInteraction)
      ...
      inline def priv: Ptr[GTlsInteractionPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTlsInteractionPrivate]]]
      inline def priv_=(value: Ptr[GTlsInteractionPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTlsInteractionPrivate]]] = value
    end extension
  
  // Allocates GTlsInteraction on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GTlsInteraction] = scala.scalanative.unsafe.alloc[GTlsInteraction](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GTlsInteractionPrivate])(using Zone): Ptr[GTlsInteraction] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GTlsInteractionPrivate]].toInt)
    res
  end offsets