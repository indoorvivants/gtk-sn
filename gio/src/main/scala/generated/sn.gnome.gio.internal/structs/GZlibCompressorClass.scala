...
object GZlibCompressorClass:
  ...
  // Allocates GZlibCompressorClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GZlibCompressorClass] = scala.scalanative.unsafe.alloc[GZlibCompressorClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass)(using Zone): Ptr[GZlibCompressorClass] =
    val ____ptr = apply()
