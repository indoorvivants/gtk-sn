...
object GInetAddressMaskClass:
  ...
  // Allocates GInetAddressMaskClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GInetAddressMaskClass] = scala.scalanative.unsafe.alloc[GInetAddressMaskClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass)(using Zone): Ptr[GInetAddressMaskClass] =
    val ____ptr = apply()
