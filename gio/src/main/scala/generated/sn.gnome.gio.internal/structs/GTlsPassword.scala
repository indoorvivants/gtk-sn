...
object GTlsPassword:
  ...
  private[internal] object fields:
    extension (struct: GTlsPassword)
      ...
      inline def priv: Ptr[GTlsPasswordPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTlsPasswordPrivate]]]
      inline def priv_=(value: Ptr[GTlsPasswordPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GTlsPasswordPrivate]]] = value
    end extension
  
  // Allocates GTlsPassword on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GTlsPassword] = scala.scalanative.unsafe.alloc[GTlsPassword](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GTlsPasswordPrivate])(using Zone): Ptr[GTlsPassword] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GTlsPasswordPrivate]].toInt)
    res
  end offsets