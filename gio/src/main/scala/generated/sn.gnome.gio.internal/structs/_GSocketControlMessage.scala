...
object _GSocketControlMessage:
  ...
  private[internal] object fields:
    extension (struct: _GSocketControlMessage)
      ...
      inline def priv : Ptr[GSocketControlMessagePrivate] = struct._2
      inline def priv_=(value: Ptr[GSocketControlMessagePrivate]): Unit = (!struct.at2 = value)
    end extension
  
  // Allocates _GSocketControlMessage on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GSocketControlMessage] = scala.scalanative.unsafe.alloc[_GSocketControlMessage](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GSocketControlMessagePrivate])(using Zone): Ptr[_GSocketControlMessage] =
    val ____ptr = apply()
