...
object GSimpleActionGroup:
  ...
  private[internal] object fields:
    extension (struct: GSimpleActionGroup)
      ...
      inline def priv: Ptr[GSimpleActionGroupPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSimpleActionGroupPrivate]]]
      inline def priv_=(value: Ptr[GSimpleActionGroupPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSimpleActionGroupPrivate]]] = value
    end extension
  
  // Allocates GSimpleActionGroup on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GSimpleActionGroup] = scala.scalanative.unsafe.alloc[GSimpleActionGroup](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GSimpleActionGroupPrivate])(using Zone): Ptr[GSimpleActionGroup] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GSimpleActionGroupPrivate]].toInt)
    res
  end offsets