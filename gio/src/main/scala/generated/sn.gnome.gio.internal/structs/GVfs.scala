...
object GVfs:
  ...
  // Allocates GVfs on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GVfs] = scala.scalanative.unsafe.alloc[GVfs](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject)(using Zone): Ptr[GVfs] =
    val ____ptr = apply()
