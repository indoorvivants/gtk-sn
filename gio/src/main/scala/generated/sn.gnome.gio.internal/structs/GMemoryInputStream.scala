...
object GMemoryInputStream:
  ...
  private[internal] object fields:
    extension (struct: GMemoryInputStream)
      ...
      inline def priv: Ptr[GMemoryInputStreamPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GMemoryInputStreamPrivate]]]
      inline def priv_=(value: Ptr[GMemoryInputStreamPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GMemoryInputStreamPrivate]]] = value
    end extension
  
  // Allocates GMemoryInputStream on the heap – fields are not initalised or zeroed out
