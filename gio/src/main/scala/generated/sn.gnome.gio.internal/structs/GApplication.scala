...
object GApplication:
  ...
  private[internal] object fields:
    extension (struct: GApplication)
      ...
      inline def priv: Ptr[GApplicationPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GApplicationPrivate]]]
      inline def priv_=(value: Ptr[GApplicationPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GApplicationPrivate]]] = value
    end extension
  
  // Allocates GApplication on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GApplication] = scala.scalanative.unsafe.alloc[GApplication](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GApplicationPrivate])(using Zone): Ptr[GApplication] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GApplicationPrivate]].toInt)
    res
  end offsets