...
object _GInetAddressMask:
  ...
  private[internal] object fields:
    extension (struct: _GInetAddressMask)
      ...
      inline def priv : Ptr[GInetAddressMaskPrivate] = struct._2
      inline def priv_=(value: Ptr[GInetAddressMaskPrivate]): Unit = (!struct.at2 = value)
    end extension
  
  // Allocates _GInetAddressMask on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GInetAddressMask] = scala.scalanative.unsafe.alloc[_GInetAddressMask](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GInetAddressMaskPrivate])(using Zone): Ptr[_GInetAddressMask] =
    val ____ptr = apply()
