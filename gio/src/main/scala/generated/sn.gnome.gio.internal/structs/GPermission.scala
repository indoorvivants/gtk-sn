...
object GPermission:
  ...
  private[internal] object fields:
    extension (struct: GPermission)
      ...
      inline def priv: Ptr[GPermissionPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GPermissionPrivate]]]
      inline def priv_=(value: Ptr[GPermissionPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GPermissionPrivate]]] = value
    end extension
  
  // Allocates GPermission on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GPermission] = scala.scalanative.unsafe.alloc[GPermission](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GPermissionPrivate])(using Zone): Ptr[GPermission] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GPermissionPrivate]].toInt)
    res
  end offsets