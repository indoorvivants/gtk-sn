...
object _GIOStream:
  ...
  // Allocates _GIOStream on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GIOStream] = scala.scalanative.unsafe.alloc[_GIOStream](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GIOStreamPrivate])(using Zone): Ptr[_GIOStream] =
    val ____ptr = apply()
