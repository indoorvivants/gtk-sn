...
object GFilterOutputStream:
  ...
  private[internal] object fields:
    extension (struct: GFilterOutputStream)
      ...
      inline def base_stream: Ptr[GOutputStream] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GOutputStream]]]
      inline def base_stream_=(value: Ptr[GOutputStream]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GOutputStream]]] = value
    end extension
  
  // Allocates GFilterOutputStream on the heap – fields are not initalised or zeroed out
