...
object GNetworkServiceClass:
  ...
  // Allocates GNetworkServiceClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GNetworkServiceClass] = scala.scalanative.unsafe.alloc[GNetworkServiceClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass)(using Zone): Ptr[GNetworkServiceClass] =
    val ____ptr = apply()
