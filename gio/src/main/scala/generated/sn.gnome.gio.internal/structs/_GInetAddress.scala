...
object _GInetAddress:
  ...
  private[internal] object fields:
    extension (struct: _GInetAddress)
      ...
      inline def priv : Ptr[GInetAddressPrivate] = struct._2
      inline def priv_=(value: Ptr[GInetAddressPrivate]): Unit = (!struct.at2 = value)
    end extension
  
  // Allocates _GInetAddress on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GInetAddress] = scala.scalanative.unsafe.alloc[_GInetAddress](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GInetAddressPrivate])(using Zone): Ptr[_GInetAddress] =
    val ____ptr = apply()
