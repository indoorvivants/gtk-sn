...
object GSimpleProxyResolver:
  ...
  private[internal] object fields:
    extension (struct: GSimpleProxyResolver)
      ...
      inline def priv: Ptr[GSimpleProxyResolverPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSimpleProxyResolverPrivate]]]
      inline def priv_=(value: Ptr[GSimpleProxyResolverPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GSimpleProxyResolverPrivate]]] = value
    end extension
  
  // Allocates GSimpleProxyResolver on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GSimpleProxyResolver] = scala.scalanative.unsafe.alloc[GSimpleProxyResolver](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GSimpleProxyResolverPrivate])(using Zone): Ptr[GSimpleProxyResolver] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GSimpleProxyResolverPrivate]].toInt)
    res
  end offsets