...
object GListStoreClass:
  ...
  // Allocates GListStoreClass on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GListStoreClass] = scala.scalanative.unsafe.alloc[GListStoreClass](1)
  def apply(parent_class : _root_.sn.gnome.gobject.internal.GObjectClass)(using Zone): Ptr[GListStoreClass] =
    val ____ptr = apply()
