...
object GEmblemedIcon:
  ...
  private[internal] object fields:
    extension (struct: GEmblemedIcon)
      ...
      inline def priv: Ptr[GEmblemedIconPrivate] = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GEmblemedIconPrivate]]]
      inline def priv_=(value: Ptr[GEmblemedIconPrivate]): Unit = !struct.at(offsets(1)).asInstanceOf[Ptr[Ptr[GEmblemedIconPrivate]]] = value
    end extension
  
  // Allocates GEmblemedIcon on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GEmblemedIcon] = scala.scalanative.unsafe.alloc[GEmblemedIcon](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GEmblemedIconPrivate])(using Zone): Ptr[GEmblemedIcon] =
    val ____ptr = apply()
  ...
  val offsets: Array[Int] =
    ...
    res(1) = align(res(0) + sizeof[_root_.sn.gnome.gobject.internal.GObject].toInt, alignmentof[Ptr[GEmblemedIconPrivate]].toInt)
    res
  end offsets