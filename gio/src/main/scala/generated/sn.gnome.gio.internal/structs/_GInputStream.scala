...
object _GInputStream:
  ...
  private[internal] object fields:
    extension (struct: _GInputStream)
      ...
      inline def priv : Ptr[GInputStreamPrivate] = struct._2
      inline def priv_=(value: Ptr[GInputStreamPrivate]): Unit = (!struct.at2 = value)
    end extension
  
  // Allocates _GInputStream on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[_GInputStream] = scala.scalanative.unsafe.alloc[_GInputStream](1)
  def apply(parent_instance : _root_.sn.gnome.gobject.internal.GObject, priv : Ptr[GInputStreamPrivate])(using Zone): Ptr[_GInputStream] =
    val ____ptr = apply()
