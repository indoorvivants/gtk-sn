...
*/
opaque type GdkKeymapKey = CStruct3[_root_.sn.gnome.glib.internal.guint, CInt, CInt]

object GdkKeymapKey:
  given _tag: Tag[GdkKeymapKey] = Tag.materializeCStruct3Tag[_root_.sn.gnome.glib.internal.guint, CInt, CInt]
  ...
  // Allocates GdkKeymapKey on the heap – fields are not initalised or zeroed out
  def apply()(using Zone): Ptr[GdkKeymapKey] = scala.scalanative.unsafe.alloc[GdkKeymapKey](1)
  def apply(keycode : _root_.sn.gnome.glib.internal.guint, group : CInt, level : CInt)(using Zone): Ptr[GdkKeymapKey] =
    val ____ptr = apply()
