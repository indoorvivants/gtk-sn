...
/** GtkResponseType: _RESPONSE_NONE: Returned if an action widget has no
  * response id, or if the dialog gets programmatically hidden or destroyed
  * _RESPONSE_REJECT: Generic response id, not used by GTK dialogs
  * _RESPONSE_ACCEPT: Generic response id, not used by GTK dialogs
  * _RESPONSE_DELETE_EVENT: Returned if the dialog is deleted _RESPONSE_OK:
  * Returned by OK buttons in GTK dialogs _RESPONSE_CANCEL: Returned by Cancel
  * buttons in GTK dialogs _RESPONSE_CLOSE: Returned by Close buttons in GTK
  * dialogs _RESPONSE_YES: Returned by Yes buttons in GTK dialogs _RESPONSE_NO:
  * Returned by No buttons in GTK dialogs _RESPONSE_APPLY: Returned by Apply
  * buttons in GTK dialogs _RESPONSE_HELP: Returned by Help buttons in GTK
  * dialogs
  */
opaque type GtkResponseType <: CInt = CInt
