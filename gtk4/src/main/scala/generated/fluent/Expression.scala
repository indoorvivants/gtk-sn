...
/** `GtkExpression` provides a way to describe references to values.
  ...
  * [func@Gtk.param_spec_expression] when creating a `GParamSpec` to install in
  * the `GObject` class being defined; for instance:
  *
  * ```c
  ...
  * [func@Gtk.value_get_expression], to retrieve the stored `GtkExpression` from
  * the `GValue` container, and [func@Gtk.value_set_expression], to store the
