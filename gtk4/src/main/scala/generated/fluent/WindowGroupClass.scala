...
import sn.gnome.gtk4.internal.GtkWindowGroupClass

class WindowGroupClass private[gnome] (raw: Ptr[GtkWindowGroupClass]):

  def getUnsafeRawPointer(): Ptr[GtkWindowGroupClass] = this.raw
...
object WindowGroupClass:
  def fromRaw(ptr: Ptr[GtkWindowGroupClass]): WindowGroupClass =
    new WindowGroupClass(ptr)
end WindowGroupClass
